...
echo "Full TESTs for QLSA Training" > full-test-training.log

################################################################################

echo "[$(date)] RUNNING QLSA - MARL" | tee -a full-test-training.log
...
echo "[$(date)] RUNNING QLSA - LateMARL" | tee -a full-test-training.log
python3 -u $RLLIB_SUMO_INFRASTRUCTURE/src/runQLSAtraining.py \
...
echo "[$(date)] RUNNING QLSA - MARLCoop" | tee -a full-test-training.log
python3 -u $RLLIB_SUMO_INFRASTRUCTURE/src/runQLSAtraining.py \
...
echo "[$(date)] RUNNING QLET - MARL" | tee -a full-test-training.log
python3 -u $RLLIB_SUMO_INFRASTRUCTURE/src/runQLSAtraining.py \
    --algo QLET --env MARL \
...
echo "[$(date)] RUNNING QLET - LateMARL" | tee -a full-test-training.log
python3 -u $RLLIB_SUMO_INFRASTRUCTURE/src/runQLSAtraining.py \
...
echo "[$(date)] RUNNING QLET - MARLCoop" | tee -a full-test-training.log
python3 -u $RLLIB_SUMO_INFRASTRUCTURE/src/runQLSAtraining.py \
...
echo "[$(date)] RUNNING PDEGQLET - MARL" | tee -a full-test-training.log
python3 -u $RLLIB_SUMO_INFRASTRUCTURE/src/runQLSAtraining.py \
...
echo "[$(date)] RUNNING PDEGQLET - LateMARL" | tee -a full-test-training.log
python3 -u $RLLIB_SUMO_INFRASTRUCTURE/src/runQLSAtraining.py \
...
echo "[$(date)] RUNNING PDEGQLET - MARLCoop" | tee -a full-test-training.log
python3 -u $RLLIB_SUMO_INFRASTRUCTURE/src/runQLSAtraining.py \
...
echo "[$(date)] Cleanup." | tee -a full-test-training.log
rm -rv res_* sumo | tee -a full-test-training.log

echo "[$(date)] Done." | tee -a full-test-training.log
