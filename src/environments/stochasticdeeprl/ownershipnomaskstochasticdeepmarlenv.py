...
from environments.stochasticdeeprl.complexstochasticdeepmarlenv import \
    ComplexStochasticPersuasiveDeepMARLEnv, ComplexDeepSUMOAgents
...
def env_creator(config):
    ...
    logger.debug('[env_creator] Environment creation: OwnershipNoMaskCSPersuasiveDeepMARLEnv')
    return OwnershipNoMaskCSPersuasiveDeepMARLEnv(config)
        ...
        if action != 0:
            ...
            if mode in self.ownership:
                ...
                if self.ownership[mode] == False:
                    ...
                    self.chosen_mode_error = 'Ownership issues for mode {}'.format(mode)
                    self.cost = float('NaN')
                    self.ett = float('NaN')
                    logger.warning('Ownership issues for agent %s. [%d - %s] [%s]',
...
class OwnershipNoMaskCSPersuasiveDeepMARLEnv(ComplexStochasticPersuasiveDeepMARLEnv):
    ...
    def craft_final_state(self, agent):
        ...
        return np.array(self.deep_state_flattener(final_state), dtype=np.float64)
    ...
    def get_observation(self, agent):
        ...
        for mode in sorted(self.agents[agent].modes):
            if mode in self.agents[agent].ownership and not self.agents[agent].ownership[mode]:
                ## it does not matter, kill this ETT
                route = None
            else:
