...
def env_creator(config):
    ...
    logger.debug('[env_creator] Environment creation: PrefChoiceRefCSPersuasiveDeepMARLEnv')
    return PrefChoiceRefCSPersuasiveDeepMARLEnv(config)

####################################################################################################

#@ray.remote(num_cpus=10, num_gpus=1)
class PrefChoiceRefCSPersuasiveDeepMARLEnv(ComplexStochasticPersuasiveDeepMARLEnv):
    ...
    def get_reward(self, agent):
        ...
        if arrival is None:
            ...
            return 0 - (max_travel_time * 2) * self.agents[agent].late_weight * pref_modifier
        ...
        if self.agents[agent].arrival < arrival:
            ...
            return 0 - (travel_time + penalty) * self.agents[agent].late_weight * pref_modifier
        ...
        if arrival_buffer > arrival:
            ...
            penalty /= 60 # in minutes
            penalty /= self._config['agent_init']['travel-slots-min'] # slotted time
            return 0 - (travel_time + penalty) * self.agents[agent].waiting_weight * pref_modifier

        #### ON TIME
        logger.info('Reward: Agent %s arrived on time.', agent)
        return 1 - travel_time * pref_modifier
