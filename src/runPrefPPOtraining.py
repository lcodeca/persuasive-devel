...
from environments.stochasticdeeprl import prefstochasticdeepmarlenv
...
def _main():
    ...
    marl_env = None
    if ARGS.env == 'MARL':
        ray.tune.registry.register_env('marl_env', prefstochasticdeepmarlenv.env_creator)
        marl_env = prefstochasticdeepmarlenv.PrefCSPersuasiveDeepMARLEnv(
            env_config)
    else:
