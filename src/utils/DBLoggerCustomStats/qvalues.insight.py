            ...
            for agent in tqdm(available_agents):
                ...
                for state, counters in state_action_counter.items():
                    for action, counter in counters.items():
                        ...
                        structure[int(time_left)][ett][int(action)] = [
                            round(qtable[state][str(action)], 1), counter]
                ...
                for pos, action in enumerate(dict_qvals):
                    ...
                    rect = ax.bar(
                        x + relpos + width / 2, dict_qvals[action], width,
                        label='{}'.format(action))
                    for p, bar in enumerate(rect):
                        if dict_counters[action][p] > 0:
                            ...
                            ax.annotate(lbl, # dict_counters[action][p],
                                        xy=(bar.get_x() + bar.get_width() / spacer,
                                            bar.get_height()),
                ...
                fig.savefig(
                    '{}.{}.{}.svg'.format(self.output_prefix, training_run, agent),
