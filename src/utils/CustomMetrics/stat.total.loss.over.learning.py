...
def _main():
    ...
    TotalLoss(config.input_dir, config.output_dir).generate()
    logging.info('Done')
...
class TotalLoss(GenericGraphMaker):
    ...
    def _aggregate_metrics(self, files):
        for filename in tqdm(files):
            ...
            with open(os.path.join(self._input_dir, filename), 'r') as jsonfile:
                ...
                self._aggregated_dataset['learning']['total_loss'].append(
                    complete['info']['learner']['unique']['total_loss'])
    ...
    def _generate_graphs(self):
        ...
        ax.plot(
            ...
            self._aggregated_dataset['learning']['total_loss'])
        ax.set(xlabel='Learning step', ylabel='Loss [#]', title='Total Loss Over Time')
        # ax.legend(loc='best', ncol=4, shadow=True)
        ax.grid()
        fig.savefig('{}/learning.total_loss_over_learning.svg'.format(self._output_dir),
