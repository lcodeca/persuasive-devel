...
def _main():
    ...
    Entropy(config.input_dir, config.output_dir).generate()
    logging.info('Done')
...
class Entropy(GenericGraphMaker):
    ...
    def _aggregate_metrics(self, files):
        for filename in tqdm(files):
            ...
            with open(os.path.join(self._input_dir, filename), 'r') as jsonfile:
                ...
                self._aggregated_dataset['learning']['entropy'].append(
                    complete['info']['learner']['unique']['entropy'])
    ...
    def _generate_graphs(self):
        ...
        ax.plot(
            ...
            self._aggregated_dataset['learning']['entropy'])
        ax.set(xlabel='Learning step', ylabel='Entropy [#]', title='Entropy Over Time')
        # ax.legend(loc='best', ncol=4, shadow=True)
        ax.grid()
        fig.savefig('{}/learning.entropy_over_learning.svg'.format(self._output_dir),
