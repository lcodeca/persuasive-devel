...
class Extractor():
    ...
    def _aggregate_metrics(self, files):
        for filename in tqdm(files):
            with open(os.path.join(self._input_dir, filename), 'r') as jsonfile:
                ...
                ### Check if this checkput improves the total reward from the previous one.
                if self._check_and_extract_tot_rewards():
                    self._extract_rewards()
                    self._extract_overview()

    def _check_and_extract_tot_rewards(self):
        self._load_aggregate_data('tot_reward')
        if self._aggregated_dataset[self._exp]:
            current = np.nansum(self._complete_metrics['hist_stats']['episode_reward'])
            best = np.sum(self._aggregated_dataset[self._exp])
            if current >= best:
                print(">>>>>>>> New BEST found: {} >= {}.".format(current, best))
                self._extract_tot_rewards()
                return True
            else:
                print("Nothing to do here: {} vs {}.".format(current, best))
                return False
        self._extract_tot_rewards()
        return True
    ...
    def _extract_tot_rewards(self):
        ...
        self._aggregated_dataset[self._exp] = deepcopy(current)
    ...
    def _extract_overview(self):
        ...
        for _metric, _mode in self._metric_to_mode.items():
            ...
            if _mode in self._mode_to_action:
                self._aggregated_dataset[self._exp] = deepcopy(avg_modes[self._mode_to_action[_mode]])
