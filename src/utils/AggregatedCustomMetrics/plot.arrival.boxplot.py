...
def _main():
    ...
    Arrival(config.input, config.output, config.outliers).generate()
...
class Arrival():
    ...
    def generate(self):
        fig, axs = plt.subplots(1, 5, figsize=(25, 15), constrained_layout=True, sharey=True)
        fig.suptitle('Arrival by Initial Distribution')

        labels, current = self._pack_data('All')
        axs[0].axhline(y=9, linestyle=':')
        axs[0].axhline(y=8.75, linestyle=':', color='g')
        axs[0].axhline(y=consts.ARRIVAL_H, color='r', label='Baseline')
        axs[0].boxplot(current, showfliers=self._outliers, showmeans=True)
        # axs[0].boxplot(
        #     current, showfliers=self._outliers, notch=False, showmeans=True, patch_artist=True,
        #     boxprops=dict(facecolor='blue', color='red'),
        #     capprops=dict(color='orange'),
        #     whiskerprops=dict(color='pink'),
        #     flierprops=dict(color='black', markeredgecolor='gray'),
        #     medianprops=dict(color='black'),
        # )
        axs[0].set(ylabel='Time [h]', title='All')
        ...
        axs[1].axhline(y=consts.ARRIVAL_H, color='r', label='Baseline')
        axs[1].boxplot(current, showfliers=self._outliers, showmeans=True)
        axs[1].set(title='30_2')
        ...
        axs[2].axhline(y=consts.ARRIVAL_H, color='r', label='Baseline')
        axs[2].boxplot(current, showfliers=self._outliers, showmeans=True)
        axs[2].set(title='30_3')
        ...
        axs[3].axhline(y=consts.ARRIVAL_H, color='r', label='Baseline')
        axs[3].boxplot(current, showfliers=self._outliers, showmeans=True)
        axs[3].set(title='45_4')
        ...
        axs[4].axhline(y=9, linestyle=':')
        axs[4].axhline(y=8.75, linestyle=':', color='g')
        axs[4].axhline(y=consts.ARRIVAL_H, color='r', label='Baseline')
        axs[4].boxplot(current, showfliers=self._outliers, showmeans=True)
        axs[4].set(title='60_5')
    ...
    #     fig.suptitle('Arrival by Reward Model')
