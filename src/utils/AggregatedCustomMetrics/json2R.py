...
def _main():
    ...
    JSON2R(config.input, config.output).process()
...
class JSON2R():
    ...
    _DEFAULT = {
        'Reward Model': None,
        'Start Distribution': None,
        'Backgound Traffic': None,
        'Experiment': None,
        'Name': None,
        'Value': None,
    }
        ...
        for experiment, values in tqdm(self._json.items()):
            ...
            for val in values:
                self._to_csv.append({
                    ...
                    'Start Distribution': _start_distr,
    ...
    def _save_to_csv(self):
        with open(self._output, 'w') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=self._DEFAULT.keys())
            writer.writeheader()
            for data in self._to_csv:
