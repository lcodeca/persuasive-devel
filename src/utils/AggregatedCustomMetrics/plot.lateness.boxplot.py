...
def _main():
    ...
    Lateness(config.input, config.output, config.outliers).generate()
...
class Lateness():
    ...
    def generate(self):
        ...
        fig.suptitle('Lateness by Initial Distribution')

        labels, current = self._pack_data('All')
        axs[0].axhline(y=0, linestyle=':')
        axs[0].axhline(y=consts.LATENESS_M, color='r', label='Baseline')
        axs[0].boxplot(current, showfliers=self._outliers, showmeans=True)
        axs[0].set(ylabel='Time [m]', title='All')
        ...
        axs[1].axhline(y=consts.LATENESS_M, color='r', label='Baseline')
        ...
        axs[2].axhline(y=consts.LATENESS_M, color='r', label='Baseline')
        ...
        axs[3].axhline(y=consts.LATENESS_M, color='r', label='Baseline')
        ...
        axs[4].axhline(y=consts.LATENESS_M, color='r', label='Baseline')
    ...
    #     fig.suptitle('Lateness by Reward Model')
