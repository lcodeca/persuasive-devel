...
def _main():
    ...
    Waiting(config.input, config.output, config.outliers).generate()
...
class Waiting():
    ...
    def generate(self):
        ...
        fig.suptitle('Waiting Time by Initial Distribution')
        ...
        axs[4].axhline(y=15, linestyle=':', color='g')
        axs[4].axhline(y=consts.WAITING_M, color='r', label='Baseline')
