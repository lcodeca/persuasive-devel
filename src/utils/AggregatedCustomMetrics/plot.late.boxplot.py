...
def _main():
    ...
    Late(config.input, config.output, config.outliers).generate()
...
class Late():
    ...
    def generate(self):
        ...
        fig.suptitle('Number of Late Agent by Initial Distribution')

        labels, current = self._pack_data('All')
        axs[0].axhline(y=0, linestyle=':')
        axs[0].axhline(y=consts.NUM_LATE, color='r', label='Baseline')
        axs[0].boxplot(current, showfliers=self._outliers, showmeans=True)
        axs[0].set(ylabel='Agents [#]', title='All')
        ...
        axs[1].axhline(y=consts.NUM_LATE, color='r', label='Baseline')
        ...
        axs[2].axhline(y=consts.NUM_LATE, color='r', label='Baseline')
        ...
        axs[3].axhline(y=consts.NUM_LATE, color='r', label='Baseline')
        ...
        axs[4].axhline(y=consts.NUM_LATE, color='r', label='Baseline')
    ...
    #     fig.suptitle('Number of Late Agent by Reward Model')

    #     labels, current = self._pack_data('wSimplifiedReward_noBGTraffic')
    #     axs[0].axhline(y=0, linestyle=':')
    #     axs[0].axhline(y=consts.NUM_LATE, color='r', label='Baseline')
    #     axs[0].boxplot(current, showfliers=self._outliers, showmeans=True)
    #     axs[0].set(ylabel='Agents [#]', title='wSimplifiedReward\nnoBGTraffic')
    #     axs[0].set_xticks(range(1, len(labels)+1)) #, minor=False)
    #     axs[0].set_xticklabels(labels, rotation=90) #, fontdict=None, minor=False)
    #     axs[0].grid()
